...
- Raise candidates across the **full in-scope attack surface — external AND
  internal.** Internal flows (service-to-service, admin, cron/batch,
  internal-only services) produce candidates the same way external flows do.
- Only skip patterns that exist solely in **excluded non-application
  artifacts** (test code, build scripts, pure IaC) — those were dropped in
  phase-1 and are not in the dataflow.
- Each data flow from Phase 2 marked HIGH or MEDIUM suspicion must produce at
  least one candidate record, regardless of its exposure.
- Copy each candidate's **Exposure** (`EXTERNAL | INTERNAL |
  INTERNAL-RESTRICTED`) from its originating phase-2 flow. Exposure does not
  gate a candidate — an internal finding is still a finding; the CVSS
  Attack Vector metric already scores network-adjacent reach lower (`AV:A`
  internal/VPN, `AV:L` local, vs `AV:N` external), so severity follows
  naturally without dropping the finding.
...
- **Exposure:** EXTERNAL | INTERNAL | INTERNAL-RESTRICTED (`references/enums.md` §17 — copied from this candidate's phase-2 flow)
