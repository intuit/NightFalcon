...
`model` argument and every reasoning override so Claude uses the model currently
...
The candidates file contains fully-populated records including: feature description,
plain-English explanation of the flaw, step-by-step attack, PoC payload, verbatim
code evidence, corrected code, and scored severity. Your job is to render those
records faithfully into well-structured prose — not to reconstruct or summarise them.
...
- `POC_MANIFEST_PATH` — path to the PoC manifest produced by phase-6
  ...
  `placeholders[]`, and reviewer `verdict`. Each PoC is self-contained
  (parameters inline); the manifest's `guide_path` points at the per-repo
  `POC-GUIDE-<DATE>.md` documenting them and `config_path` at the per-repo
  reference catalog. Read once at the start of phase-7;
  ...
  entry (PoC generated) or a `skipped[]` entry (skip reason) — render
  the matching section for each and never omit it. Only DISMISSED
  findings and P3/P4 findings land in `skipped[]` with a not-eligible
  reason; CONFIRMED, CONFIRMED-MODIFIED, and NEEDS-REVIEW P0/P1/P2
  findings all get a PoC.
...
- `ORGANIZATION_CONTEXT_PATH` — optional path to the organization context file
  ...
  repo (or the sentinel `No organization controls detected.`). Use it to
  cite the recommended control and applicable STD in every `How to
  fix it` block where the finding overlaps a detected control. It is
  omitted in source-less triage; do not invent an organization control or STD
  when the imported candidate did not supply one. The
...
- `FINDINGS_JSON_PATH` — path this worker writes for the schema-v2 findings JSON.
- `RECEIPT_PATH` — path this worker writes for the validator-bound receipt.
- `PATTERN_TAGS_PATH` — path this worker writes for the pattern-tag projection.

Render every non-dismissed candidate exactly once from its post-debate candidate
record. Deterministic projection never removes a supported finding. In source-less
triage, use `OWASP_CONTEXT_PATH` and `ORGANIZATION_CONTEXT_PATH` when available; when
...
- P4-downgraded findings (PATCHED validation outcome) go in P4 (Informational).
...
- **What kind of issue this is:** <one-sentence plain-English taxonomy
  followed by **the framework + item it maps to**. Every finding must
  cite at least one taxonomy. Selection rules (apply in order, first
  match wins):
...
- **Exposure:** EXTERNAL | INTERNAL | INTERNAL-RESTRICTED — <one short phrase
  in plain English, e.g. "External — reachable by any authenticated example SaaS application user"
  or "Internal — service-to-service only, reachable from a compromised peer" or
  "Internal-restricted — VPN / internal-network only". Copied from the
  candidate's Exposure field (`references/enums.md` §17).>
- **Severity (CVSS v4.0 Base):** <CVSS-B numeric score> — `<full CVSS:4.0/... vector>` ([calculator](https://www.first.org/cvss/calculator/4.0#<vector>)). The P-tier is derived from this score (P0 9.0-10.0 … P4 0.0). Copy the score and vector verbatim from the candidate record; do not re-score here. **Every finding has a score — this line is never omitted.**
- **Classification:** <CWE-<NNN> (<name>) when a CWE fits; CVE-<YYYY>-<NNNN> for a dependency-CVE finding; plus the OWASP/category citation. Show every identifier that applies; write "no single CWE" only when none genuinely fits (the OWASP citation still appears in the Category line).>
...
script path and state the reviewer verdict; (b) name the inline
parameters this self-contained script needs and link the shared guide.
Format:

  An executable, self-contained proof-of-concept is available at
  [`<script_path>`](<script_path>) (`<script_type>`; reviewer verdict:
  **<verdict>**). Its parameters are set inline at the top of the script
  (or via env vars) — it needs: `<KEY1>`, `<KEY2>`, … . See
  [`<guide_path>`](<guide_path>) for the full parameter list. Run only
  against a non-production environment you control.

`<guide_path>` comes from the manifest's top-level `guide_path` (the
per-repo `POC-GUIDE-<DATE>.md`); `<KEY1>, <KEY2>, …` is the comma-joined
`placeholders[]` for this PoC. Do NOT tell the developer to "fill a
config file" — the PoC is self-contained.
...
- `confirm_path: "browser"` → "This PoC is **browser-driven** — run it
  under Claude with the chrome-devtools MCP (it cannot be confirmed by a
...
| `not-eligible-disposition` | "this finding was DISMISSED in debate; executable PoCs are generated for every other P0/P1/P2 disposition." |
...
- **Validation:** <CURRENT / PATCHED / UNVERIFIED> — <plain-English note from phase-5>
...
- **Browser-driven** (`confirm_path: "browser"`) — "These PoCs require a
  rendered browser and must be run under Claude with the chrome-devtools
  MCP; the single invoker marks them `SKIPPED-AGENT`." For each:
...
Each PoC is self-contained — set its inline parameters (see the repo's
`POC-GUIDE-<DATE>.md`) or export them, then run the script directly. To run
every script-confirmed PoC at once, the convenience runner
`output/proof_of_concept/run-all.sh` executes them all.

---

## Methodology Notes
- Scope: full attack surface — both external (public-internet / authenticated-
  external) and internal (service-to-service, admin, cron/batch, internal-only
  services) application surface analyzed. Each finding is labeled with its
  exposure (EXTERNAL / INTERNAL / INTERNAL-RESTRICTED). Only non-application
  artifacts (test code, build scripts, pure IaC) are excluded.
...
- Proof-of-concept generation: one runnable PoC per P0/P1/P2 finding not
  DISMISSED in debate — CONFIRMED, CONFIRMED-MODIFIED, and NEEDS-REVIEW all
  get scripts (.sh / .py, or a chrome-devtools browser-recipe only when a
  script cannot confirm), two-reviewer static verification (verdict: VALID /
  NEEDS-REVISION / INVALID). All PoCs share one root config and one invoker
...
- When `pocs[]` has an entry: copy `script_path`, `script_type`,
  ...
  manifest entry; copy `guide_path` (the per-repo `POC-GUIDE-<DATE>.md`)
  and `config_path` (the per-repo reference catalog
  `output/proof_of_concept/<slug>/poc-config.env`) from the manifest's
  top-level fields. (For a legacy v3 manifest with no `guide_path`, omit
  it.)
...
Phase-8's HTML renderer surfaces the first form as a clickable link to
the script plus a "self-contained — set these params inline: KEY1, KEY2
(see the guide)" line, and the second form as a muted "Not generated —
<reason>" line.
...
5. **Write via the Write tool.** This JSON, the receipt, and pattern tags
   are the worker-authored artifacts; Markdown and SARIF are deterministic
   orchestrator projections.
6. **Schema is now `"2"`.** Emit `"schema_version": "2"` — v2 adds the
   top-level provenance fields and the required per-finding CVSS.
7. **Copy `repo_url`, `commit_sha`, and `multitenant_scope` verbatim from
   `receipt-<DATE>.json`.** These top-level fields let automation and
   ticketing key off the exact commit without re-reading the receipt.
   In triage mode use `commit_sha: "n/a (triage)"` and mirror the
   receipt's `multitenant_scope`.
8. **Every finding MUST carry a non-empty `cvss_score` (0.0-10.0) and a
   complete `cvss_vector` full Base vector (`CVSS:4.0/...`).** Copy both from the
   candidate, then verify them with `python3 scripts/cvss_v4.py '<vector>'`.
   The phase-7 gate independently recalculates the score and blocks malformed
   vectors, score/vector mismatches, or a `tier` outside the score's mechanical
   band (enums.md §18). No tier override or P0 floor exists.
9. **`cwe` and `cve` keys are always present.** Use a valid `CWE-NNN` when
   a weakness maps cleanly and a valid `CVE-YYYY-NNNN` only for a specific
   dependency vulnerability; otherwise use JSON `null`. CVSS remains required
   and canonical for every finding.
10. **`exposure` is the canonical enum, not prose.** Emit exactly one of
    `EXTERNAL | INTERNAL | INTERNAL-RESTRICTED` (enums.md §17) — never a
    sentence. The gate blocks a non-enum value.
11. **`reachability`, `exploitability`, `impact`, and `patch_status`
    mirror the four `severity_md` bullets.** Each carries the same one
    sentence as its matching `Reachability.` / `Exploit difficulty.` /
    `Impact on success.` / `Patch status.` bullet, extracted as a flat
    string so automation reads it without parsing `severity_md`.
