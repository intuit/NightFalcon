#!/usr/bin/env python3
"""Resolve active NightFalcon workspace from a Claude hook event."""
...
def resolve_workspace(
    ...
    """Return nearest active workspace, bounded by client project root when known."""
    ...
    roots: list[Path] = []
    for value in (payload.get("workspace_root"), env.get("CLAUDE_PROJECT_DIR")):
        root = _resolved(value)
        if root is not None:
            roots.append(root)
    raw_roots = payload.get("workspace_roots")
    if isinstance(raw_roots, list):
        roots.extend(root for value in raw_roots if (root := _resolved(value)) is not None)

    containing = [root for root in roots if _contains(root, cwd)]
