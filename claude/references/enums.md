...
| generation-failed | agent-budget-exhausted | no-candidates
```
...
- `not-eligible-disposition` — the finding's disposition is
  `DISMISSED` (disproven in debate). Every other P0/P1/P2
  disposition — `CONFIRMED`, `CONFIRMED-MODIFIED`, and
  `NEEDS-REVIEW` — gets a PoC (NEEDS-REVIEW included since v2.1.0:
  the script is the instrument that resolves the open question).
- `generation-refused` — phase-6 attempted but the model declined to
  produce an executable script for this attack class. The refusal is
  final for the run: the finding stays in `skipped[]` and is **never
  retried on a different model** (the v1.x phase-6 fallback chain was
  removed in v2.0.0 — the model changes only on explicit user
  instruction, never on refusal). The `note` carries the verbatim
  refusal sentence.
- `generation-failed` — write/tool error during generation. The skip
  `note` carries the error message.
- `agent-budget-exhausted` — the harness's per-session subagent spawn
  budget (default 150, `NIGHTFALCON_MAX_AGENTS`) ran out before this
  finding's blind reviewers could be spawned, so the PoC could not be
  adversarially verified. An unreviewed PoC is never shipped in
  `pocs[]`. The `note` carries the path of the generated-but-unreviewed
  script if one was written; a resumed session (fresh budget) may
  re-run the review.
