...
copied from its phase-2 flow — internal findings are raised, not dropped — plus
a CVSS v4.0 Base **Score** + **Vector** (enums.md §18, canonical severity; the
tier is derived from the score) and an optional **CWE** id (enums.md §19)).
...
the phase-4 conversation — it always runs as a fresh, context-isolated Cursor
subagent, one per round/finding, never reused. The phase-4 worker waits for
each required DA; a spawn
...
as the DA. Each reviewer is run as a fresh, context-isolated Cursor subagent,
one per finding, never reused; the phase-6 worker waits for both:
