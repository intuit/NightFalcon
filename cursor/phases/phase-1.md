...
`$PWD/references/OWASP/_graph.json` (regenerated by
...
```bash
: "${PLUGIN_ROOT:?PLUGIN_ROOT must point to the NightFalcon Cursor plugin root}"
STATUS_LOG="$WORKSPACE/findings/$REPO_SLUG/callgraph-status-${DATE}.md"

for lang in java javascript python go; do
  (
    python3 "$PWD/scripts/callgraph/build-callgraph.py" \
