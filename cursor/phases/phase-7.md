...
- `ORGANIZATION_CONTEXT_PATH` — optional path to the organization context file
  ...
  `$PWD/references/organization_context/` — read control or
  ...
  **Context-provider read discipline (token-frugal).** The phase-1 context
  ...
  `$PWD/references/organization_context/_graph.json` only
...
- `confirm_path: "browser"` → "This PoC is **browser-driven** — run it
  via Cursor with available browser/computer-use control (it cannot be confirmed by a
...
- **Browser-driven** (`confirm_path: "browser"`) — "These PoCs require a
  rendered browser and must run via Cursor with available browser/computer-use
...
6. **`schema_version` is now `"2"`.** The v2 top-level adds `repo_url`,
   `commit_sha`, and `multitenant_scope` — copy all three **verbatim** from
   `receipt-<DATE>.json` (do not re-derive them). The phase-7 gate blocks
   a missing or empty `repo_url` / `commit_sha`.
7. **Every finding MUST carry a non-empty `cvss_score` (0.0-10.0) and a
   complete `cvss_vector` full Base vector or the gate blocks.** Copy both from the
   candidate, then verify them with `python3 scripts/cvss_v4.py '<vector>'`.
   The gate independently recalculates the score and blocks malformed vectors,
   score/vector mismatches, or a `tier` outside the score's mechanical band.
   No tier override or P0 floor exists.
8. **`cwe` and `cve` keys are always present.** Use valid identifiers when
   applicable; otherwise use JSON `null`. CVSS remains required and canonical
   for every finding.
9. **`exposure` is the canonical enum** (`EXTERNAL | INTERNAL |
   INTERNAL-RESTRICTED`, enums.md §17), never a prose sentence. The gate
   blocks a non-enum value when the field is set.
10. **`reachability`, `exploitability`, `impact`, and `patch_status`
    mirror the four `severity_md` bullets** (Reachability / Exploit
    difficulty / Impact on success / Patch status), extracted as one
    sentence each so automation can read them without parsing `severity_md`.
...
```bash
python3 "$PWD/scripts/report/build-sarif.py" \
