...
**Default batch size: `5` findings concurrent.**
...
3. **Round-by-round, batch-synchronous execution.** For round `r` from 1
   ...
   - Each DA returns its typed envelope independently (see "Typed return
     envelopes" below). Capture the terminal response before evaluating or
     replacing that DA. Envelopes are independent — no
     shared state across findings.
...
- **Per-repo Phase-4 workers run sequentially** (see SKILL.md). Per-repo
  workers run sequentially (one repo slug at a time); only the finding batch
  inside a single repo fans out.

If `PARALLEL_BATCH_SIZE` is not provided in the input variables, default
to `5` (clamp `[1, 8]`). If the worker observes subagent
...
2. **Spawn a NEW, isolated DA subagent** as a fresh, context-isolated Cursor
   subagent (Agent/Task tool). Each spawn and re-spawn is context-isolated:
   pass only the documented blind DA packet below, with no accumulated
   conversation. Omit the `model` argument and every reasoning
   override. Cursor then uses the model currently selected by the user.
   ...
   **DA isolation is a hard rule:**
   - The DA MUST run in its **own conversation**. NEVER evaluate the DA
     ...
     across rounds or across findings. **One fresh, context-isolated Cursor
     subagent (Agent/Task tool) = one fresh DA.** Wait for the DA before
     adjudicating this finding's round.
   - **Pass ONLY:**
   ...
   **If a fresh spawn or wait operation fails for any reason:** return a
   runtime block for phase-4. Never evaluate the DA inline or reuse another
   DA's conversation as a fallback. If the subagent tool is unavailable,
   report and stop.
   ...
   **For every normal typed DA envelope:** require
   ...
   response and spawn a fresh blind leaf with the
...
For every candidate marked `Imported finding: YES`, independently derive a
CVSS v4.0 Base vector and score during debate (per `references/cvss-policy.md`)
and derive `Final Tier` from that score; `tier_claimed` is provenance only and
is never accepted as Phase-3 scoring. Always write the derived CVSS-B
vector+score and the final tier even when the tier happens to equal the
imported claim.

For ordinary-review candidates, if the DA's challenges changed any CVSS metric
(e.g. `VC:H` → `VC:L` after Primary shows bounded impact, or `AV:N` → `AV:A`
after Primary concedes the sink is not internet-reachable), recompute the
CVSS-B score from the revised vector and re-derive the tier. Record both the
initial and final tier in the candidate record; the receipt's `tier_initial`
and `tier_final` fields make the change auditable.

The DA must challenge any `VC:H` or `VI:H` on a credential-leak, missing-authz,
or BOLA/IDOR finding — high confidentiality/integrity impact is asserted, not
assumed, and the DA forces Primary to justify the metric against bounded
real-world impact. The DA must also challenge any secret scored `VC:H` that
lacks the four secret-calibration links (non-public, active, guards a security
boundary, and named the operations it authorizes — see
`references/cvss-policy.md`); a secret missing any link does not hold `VC:H`.
