...
If no CONFIRMED, CONFIRMED-MODIFIED, or NEEDS-REVIEW P0/P1/P2 candidate
requires validation, do not make network requests. Append this block once to
`<CANDIDATES_PATH>` and return:

```markdown
## Phase 5 Validation
NO_VALIDATION_CANDIDATES
- Validation: NOT-RUN
- Validation source: n/a
- Validation note: No CONFIRMED, CONFIRMED-MODIFIED, or NEEDS-REVIEW P0/P1/P2 candidate required validation.
```
...
The eligible candidates (CONFIRMED, CONFIRMED-MODIFIED, or NEEDS-REVIEW at P0 / P1 / P2)
form a queue. Process the queue in **batches of
...
2. **Route to one of three validation paths based on `finding_type` and
   ...
   ### Path A — dep-CVE validation (real web search / fetch)
   ...
   ### Path B — pattern-class lookup (static citation table, no web search)
   ...
   provenance. **Do NOT run a web search for these.**
   ...
   **Path B never runs a web search and never hits NVD rate limits.**
