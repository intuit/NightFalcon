...
- `POC_REVIEWER_PROMPT_PATH` — path to the blind PoC reviewer subagent
  prompt (`$PLUGIN_ROOT/phases/phase-poc-reviewer.md`)
...
- `MAX_AGENT_THREADS` — file-resolved configured `agents.max_threads` from
  ...
  `PARALLEL_BATCH_SIZE` to at most
  `floor((MAX_AGENT_THREADS - 1) / 2)`, reserving this Phase-6 worker thread
  and two reviewer threads per finding.
- For every `pocs[].generated_by`, record the actual advisory Codex model
...
required. The recipe is a declarative JSON document a Codex agent with
...
subagents **in parallel** with fresh Codex nested-agent operations. Each reviewer is blind to
the other's existence and verdict — same isolation discipline as the
phase-4 Devil's Advocate.

Every PoC reviewer spawn and re-spawn MUST set `fork_turns="none"`. Pass only the documented blind reviewer packet below. Never accept the default context
fork, pass the Phase-6 conversation, or evaluate either reviewer inline.
...
Replace every `<workspace>` below with the absolute `WORKSPACE` input on each
call; Codex shell exports do not persist between tool calls.
...
- Omit the `model` argument and every reasoning override. Codex then uses the
...
Wait for both reviewers before resolving verdict. Capture and validate each
terminal response, then apply mode-specific lifecycle handling before resolving
verdict or spawning replacements. Before spawning, set
`AGENT_LIFECYCLE_MODE=explicit-close` when `close_agent` is callable. When it is
absent, require `interrupt_agent` and `followup_task`, then set
`AGENT_LIFECYCLE_MODE=auto-evict`.
In `explicit-close` mode, close the terminal thread before recording its terminal
journal event and block if close fails. In `auto-evict` mode, record terminal
event and never call unavailable tool; v2 auto-evicts terminal agents when
capacity is needed. Register every successful spawn in `ACTIVE_AGENT_IDS`, and
remove it only after terminal response and mode-specific handling complete.
For any fresh-spawn failure other than exact `agent thread limit reached`,
record `agent-failed` for requested attempt with `status-code failed` and
`reason-code spawn-rejected`, then block without retrying. On a wait failure in
`explicit-close` mode, call `close_agent`, record `agent-failed`, and remove
active ID only after close succeeds, then block. On a wait failure in
`auto-evict` mode, call `interrupt_agent` to stop child's current turn, use
`followup_task` with a no-tool cleanup instruction, and wait for that turn's
terminal response. Only then record `agent-failed`, remove ID, and block. Never
treat interrupt itself as terminal. If cleanup cannot finish, retain active ID
and started journal attempt, block root run, and require resume from a new root
task. Never review script inline. If a reviewer returns single line
`BLIND_VIOLATION`, discard that response, finish mode-specific terminal
handling, strip leaked context, and re-spawn that reviewer fresh.
Cap isolation re-spawns at two per reviewer; a third violation is a runtime
block.

If spawn returns `agent thread limit reached`, do not immediately retry. Record
`agent-failed` for requested attempt with no runtime ID using `status-code
failed` and `reason-code spawn-rejected`, then record `retry-scheduled` and mint
a new `CHILD_LEDGER_ID`. Retain every successful child ID,
its finding/reviewer/round association, and any valid result already captured.
Stop new fan-out, wait for every successful sibling still running, capture its
result, and apply mode-specific terminal handling to each sibling; retry only the failed child queue item once
and use `PARALLEL_BATCH_SIZE=1` for future refills
in this repo worker. Never re-spawn a successful sibling or duplicate its
result. If one retry returns same error, return a runtime block.
...
If the field is missing or not a boolean, discard malformed response after
it becomes terminal and spawn a fresh blind leaf with same authorized
...
Before returning from phase-6, require `ACTIVE_AGENT_IDS` to be empty. Drain
any remaining reviewer through same wait, terminal journaling, and
...
3. Spawn two **fresh** reviewer subagents (new nested-agent operations; do not reuse
...
`min(3, floor((MAX_AGENT_THREADS - 1) / 2))`, clamp `[1, 5]` and to the
...
If you observe nested-agent fan-out failures or rate-limit responses,
...
`.browser.json` recipes `SKIPPED-AGENT` (they are driven by a Codex
...
run_one() {
  ...
  case "$type" in
    ...
    browser)
         echo "agent-driven — run via Codex with available browser/computer-use control (see $f)" >"$out/$fid.log"
...
echo "================================================"
echo "Browser-driven (SKIPPED-AGENT) PoCs must run via Codex with available"
  ...
  "pocs": [
    {
      ...
      "generated_by": "<actual advisory Codex model reported to this worker, or unknown>",
      "verdict": "VALID",
      "regenerated": false,
      "reviewer_envelopes": [
        {
...
override, so Codex uses the model currently selected by the user. Do not
...
1. **Generation succeeds** → write the script to `<POC_DIR>` (Step
   ...
   entry whose `generated_by` is the actual advisory Codex model reported to
