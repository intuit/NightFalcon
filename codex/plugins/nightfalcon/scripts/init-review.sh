...
# current Codex selection; already-running agents keep their launch model.
#
# Exit codes:
#   0 — Success
#   1 — Usage error
...
# Mark this workspace as an active NightFalcon Codex review so the runtime
# hooks enforce phase gates only inside initialized review workspaces.
printf 'nightfalcon\n' > "$WORKSPACE/.nightfalcon-review"

# Record the installed Codex plugin version in run metadata when the plugin
# root is supplied by the orchestrator.
PLUGIN_VERSION="unknown"
if [[ -n "${PLUGIN_ROOT:-}" && -f "${PLUGIN_ROOT}/.codex-plugin/plugin.json" ]]; then
  PLUGIN_VERSION="$(python3 -c 'import json, sys; print(json.load(open(sys.argv[1])).get("version", "unknown"))' "${PLUGIN_ROOT}/.codex-plugin/plugin.json" 2>/dev/null || echo unknown)"
fi
